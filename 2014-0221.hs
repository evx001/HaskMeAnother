...
GC				mark and sweep 		refcounting 
----------------------------------------------------------------------

Colllecltion style		batch 			Incremental 
...
Mark and Sweep
	...
	Generational Collection a 90s improvement
...
Other alogorythmic Duals 
primal problem 
	dual problem
 
primal dual 

Partition Divided memory (heap, stack and global variable) into a set of partitions within which different stratagies may be applied. 

Traversal For each partition decide whether the object graph will be traversed by tracing or reference counting and 

Trade-offs For each partitiion, choose space-time trade-offs such as semi-space vs sliding compaction, pointer reversal vs stack traversal etc. 
