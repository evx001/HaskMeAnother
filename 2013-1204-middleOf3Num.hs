-- C:\Users\evxasus001>cd Documents\GitHub\HaskMeAnother

-- Next replace import with a single line deal that does same 

module MidOf3Num where
import ListSort

-- midOf3Num  x y z  = [x,y,z] 
-- midOf3Num a b c   = ([head xs | xs <- [listSort ls | ls <- [midOf3Num a b c]]])

           
-- *MidOf3Num> ([head xs | xs <- [listSort ls | ls <- [midOf3Num 4 2 5]]])!!0
--      2
-- *MidOf3Num> head([head xs | xs <- [listSort ls | ls <- [midOf3Num 4 2 5]]])
--      2          
{-   
*MidOf3Num> ([head xs | xs <- [listSort ls | ls <- [midOf3Num 4 2 5]]])!!0
[2]
...
--  let midOf3Num a b c = ([head xs | xs <- [listSort ls | ls <- [midOf3Num a b c]]])
