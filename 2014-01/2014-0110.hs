...
-- NOTE!!! in GHC newly compiled modules aren't picked up by :reload, only :load:
-- 6.1 -- 
...
-- 6.8.1 -- 
-- Define the exponentiation operator ^ for non-neg integers like the (*) then show how 2^3 is evaluated 
-- (^)
let m * 0           = 1 
    where m ^ (n+1)       = m * m ^ n 
>>>>>>> 303901fbb168a30162ae48db9a37b3c08b3c8417
